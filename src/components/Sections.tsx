...
export function VehicleRange() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // Header stays visible throughout the section
      if (header) {
        gsap.fromTo(
          header,
          { y: 35, opacity: 0 },
      ...
      // Row-by-Row 3 cards per row: Entrance triggers when scrolled into view, stays visible until user leaves section
  ...
  return (
    <section id="vehicles" ref={sectionRef} className="relative border-t border-slate-200 bg-white py-24 md:py-32">
